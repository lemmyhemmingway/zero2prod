...
async fn spawn_app() -> TestApp {
    ...
    let server =
        zerotoprod::startup::run(listener, connection_pool.clone()).expect("Failed to bind address");
...
pub async fn configure_database(config: &DatabaseSettings) -> PgPool {
    let mut connection = PgConnection::connect(&config.connection_string_without_db()).await.expect("Failed to connect postgres");
...
async fn health_check_works() {
    ...
    let response = client
        .get(format!("{}/healthcheck", &app.db_address))
        .send()
        .await
...
async fn subscribe_returns_a_200_for_a_valid_form_data() {
    ...
    let response = client
        .post(&format!("{}/subscriptions", &app.db_address))
